  ...
  module Schema
    ...
    def init_stagehand!(options = {})
      ActiveRecord::Schema.define do
        ...
        add_index :stagehand_commit_entries, [:operation, :commit_id] # Used for looking up start entries
      end

      add_stagehand!(options)
    end

    def add_stagehand!(force: false, **options)
      ActiveRecord::Schema.define do
    ...
    def create_operation_trigger(table_name, trigger_event, record, force: false)
      ...
      create_trigger(table_name, trigger_event, :after, <<-SQL)
        BEGIN
          INSERT INTO stagehand_commit_entries (record_id, table_name, operation, commit_id)
          VALUES (#{record}.id, '#{table_name}', '#{trigger_event}', @stagehand_commit_id);
        END;
      SQL
    end
